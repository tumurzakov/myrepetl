...
class TestTransformService:
    ...
    def test_apply_column_transforms_with_transform(self):
        ...
        with patch('importlib.import_module') as mock_import:
            ...
            result = service.apply_column_transforms(row_data, column_mapping, "test_schema.test_table")
            
            assert result == {"id": 1, "name": "TRANSFORMED"}
            mock_function.assert_called_once_with("test", {"id": 1, "name": "test"}, "test_schema.test_table")
